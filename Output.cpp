...
void Output::DrawPlayer(const CellPosition & cellPos, int playerNum, color playerColor) const
{
	...
	if (playerNum >= MaxPlayerCount || playerNum < 0)
		return;
	...
	pWind->DrawCircle(x, y, radius);                                                                        //!!!!!!111111111
...
void Output::DrawLadder(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	if (fromCell.HCell() != toCell.HCell() || fromCell.VCell() < toCell.VCell() || fromCell.VCell() == 0)
		return;
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	if (fromCell.HCell() != toCell.HCell() || fromCell.VCell() > toCell.VCell() || fromCell.VCell() == 8)
		return;
	...
	pWind->DrawLine(x12, y1, x12 - 1, y2 - 1);
